...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # phases: 0 at start/end of swing, 1 at swing apex
        phases = 1 - torch.abs(1.0 - torch.clamp((self.foot_indices * 2.0) - 1.0, 0.0, 1.0) * 2.0)

        # foot heights in world frame (N,4)
        foot_heights = self.foot_positions_w[:, :, 2]

        # target height: 8cm at apex + ~2cm offset
        target_height = 0.08 * phases + 0.02

        # penalize deviation during swing (desired_contact_states ~ 0)
        rew_foot_clearance = torch.square(target_height - foot_heights) * (1.0 - self.desired_contact_states)
        rew_feet_clearance = torch.sum(rew_foot_clearance, dim=1)  # (N,)



        # contact forces for feet from sensor (N,4)
        ...
        for i in range(4):
            # when desired_contact ~ 0 (swing), penalize contact force
        ...
        ### aDDITION COMPLETLY  ENDS 1.4 GET_REWARDS
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        self.gait_indices[env_ids] = 0.0  ## This one is part 4 addition 
    ...
    ## Implementation of the control logic 2.3 part starts here 
    ...
    def _apply_action(self) -> None:
        ...
        self.robot.set_joint_effort_target(torques)
            ## Implementation of the control logic 2.3 part ends  here 
